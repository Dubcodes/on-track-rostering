...
- `app/catalog`: Admin global/region policy administration plus Manager-scoped regional tracks and stable palette-slot allocation; crew groups, base positions, and vehicles remain global catalogues. Active tracks have distinct slots 1–20 per region. Allocation locks the destination Region within the caller's transaction; a partial unique index is the final database guard. Track UUIDs are identity; theme CSS owns colours, not master data or revisions.
